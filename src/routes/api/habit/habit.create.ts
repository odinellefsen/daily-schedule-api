import type { Hono } from "hono";
import z from "zod";
import { batchHabitCreationSchema } from "../../../contracts/habit/habit.contract";
import { ApiResponse, StatusCodes } from "../../../utils/api-responses";
import { FlowcorePathways } from "../../../utils/flowcore";

export function registerCreateHabit(app: Hono) {
    app.post("/batch", async (c) => {
        const safeUserId = c.userId!;

        const rawJsonBody = await c.req.json();
        const parsedJsonBody = batchHabitCreationSchema.safeParse({
            ...rawJsonBody,
            userId: safeUserId,
        });

        if (!parsedJsonBody.success) {
            return c.json(
                ApiResponse.error(
                    "Invalid batch habit creation data",
                    parsedJsonBody.error.errors,
                ),
                StatusCodes.BAD_REQUEST,
            );
        }

        const safeBatchHabitData = parsedJsonBody.data;

        try {
            await FlowcorePathways.write("habit.v0/habits.created.v0", {
                data: safeBatchHabitData,
            });
        } catch (error) {
            return c.json(
                ApiResponse.error("Failed to create instruction habits", error),
                StatusCodes.SERVER_ERROR,
            );
        }

        return c.json(
            ApiResponse.success("Instruction habits created successfully", {
                mealId: safeBatchHabitData.mealId,
                mealName: safeBatchHabitData.mealName,
                habitCount: safeBatchHabitData.habits.length,
            }),
            StatusCodes.CREATED,
        );
    });
}
