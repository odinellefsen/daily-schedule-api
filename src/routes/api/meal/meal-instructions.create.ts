...
meal.post("/instructions", async (c) => {
    ...
    const existingInstructions = await db
        .select()
        .from(mealSteps)
        .where(eq(mealSteps.mealId, safeCreateMealInstructionsJsonBody.mealId));
    ...
    const safeCreateMealInstructionsEvent = createMealInstructionsEvent.data;
