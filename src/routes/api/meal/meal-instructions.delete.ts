...
meal.delete("/instructions", async (c) => {
    ...
    const safeDeleteMealInstructionsRequestBody = parsedRequestJsonBody.data;
    ...
    const existingInstructions = await db
        ...
        .from(mealSteps)
        .where(
            eq(mealSteps.mealId, safeDeleteMealInstructionsRequestBody.mealId)
        );
    ...
    try {
        await FlowcorePathways.write("meal.v0/meal-instructions.archived.v0", {
