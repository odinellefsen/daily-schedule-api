...
meal.get("/week", async (c) => {
    ...
    for (const mealData of upcomingMeals) {
        ...
        const completedSteps = allSteps.filter((step) => step.isStepCompleted);
        const recipes = JSON.parse(mealData.recipes);

        const mealWithProgress = {
            mealId: mealData.id,
            mealName: mealData.mealName,
            scheduledToBeEatenAt: mealData.scheduledToBeEatenAt?.toISOString(),
            ...
            nextStep:
                allSteps.find((step) => !step.isStepCompleted)?.instruction ||
                null,
...
meal.get("/", async (c) => {
    ...
    return c.json(
        ApiResponse.success("Meals retrieved successfully", mealsWithRecipes)
    );
});

meal.get("/:mealId", async (c) => {
    const safeUserId = c.userId!;
    ...
    const fullMeal = {
        ...
        scheduledToBeEatenAt: mealFromDb.scheduledToBeEatenAt?.toISOString(),
        ...
        nextStep:
            steps.find((step) => !step.isStepCompleted)?.instruction || null,
        estimatedTimeRemaining: steps
            .filter(
                (step) => !step.isStepCompleted && step.estimatedDurationMinutes
            )
            .reduce(
    ...
    return c.json(ApiResponse.success("Meal retrieved successfully", fullMeal));
});
