...
recipe.delete("/ingredients", async (c) => {
    ...
    const safeDeleteRecipeIngredientsRequestBody = parsedRequestJsonBody.data;

    // Verify recipe exists and belongs to user
    const recipeFromDb = await db.query.recipes.findFirst({
        where: eq(recipes.id, safeDeleteRecipeIngredientsRequestBody.recipeId),
    });
    ...
    try {
        ...
        return c.json(
            ApiResponse.error("Failed to archive recipe ingredients", error),
