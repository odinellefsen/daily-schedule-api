...
recipe.patch("/ingredients", async (c) => {
    ...
    const safeUpdateRecipeIngredientsRequestBody = parsedRequestJsonBody.data;

    // Verify recipe exists and belongs to user
    const recipeFromDb = await db.query.recipes.findFirst({
        where: eq(recipes.id, safeUpdateRecipeIngredientsRequestBody.recipeId),
    });
    ...
    const safeUpdateRecipeIngredientsEvent = updateRecipeIngredientsEvent.data;
