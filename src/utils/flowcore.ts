import type { PathwayRouter, PathwaysBuilder } from "@flowcore/pathways";
...
let flowcorePathwaysInit: Promise<PathwaysBuilder> | undefined;

export function getFlowcorePathways(): Promise<PathwaysBuilder> {
    // `@flowcore/pathways`' CJS build requires `file-type` which is ESM-only in v21,
    // and will crash on Vercel if we import it eagerly in a CJS runtime.
    // Using dynamic import forces Node to use the package's ESM export.
    flowcorePathwaysInit ??= (async () => {
        const { createPostgresPathwayState, PathwaysBuilder } = await import(
            "@flowcore/pathways"
        );
    ...
    return flowcorePathwaysInit;
}

let pathwaysRouterInit: Promise<PathwayRouter> | undefined;

export function getPathwaysRouter(): Promise<PathwayRouter> {
    pathwaysRouterInit ??= (async () => {
        const { PathwayRouter } = await import("@flowcore/pathways");
        const pathways = await getFlowcorePathways();
        return new PathwayRouter(pathways, webhookApiKey);
    })();

    return pathwaysRouterInit;
}
